      ...
      <div className="fixed inset-y-0 right-0 flex max-w-full pl-0 sm:pl-10">
        <div className="w-screen max-w-md bg-[#f9f9f7] border-l border-[#e5e5e3] flex flex-col shadow-2xl">
          ...
          <div className="px-6 py-6 border-b border-[#e5e5e3] flex items-center justify-between bg-[#ffffff]">
            <div>
              <span className="text-xs uppercase tracking-[0.25em] text-[#595D5D] font-medium block">
                Acquisition Drawer
              </span>
          ...
          {notice && (
            <div className="bg-[#fff9ee] border-b border-[#ebd7b2] px-6 py-2.5 text-xs text-[#8a681c] flex items-center justify-between">
          ...
          <div className="flex-1 overflow-y-auto px-6 py-6 divide-y divide-[#e5e5e3]">
            {items.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center text-center py-16">
                <div className="w-12 h-12 border border-[#e5e5e3] flex items-center justify-center text-[#595D5D] mb-4">
                  <Package className="w-6 h-6 stroke-[1.2]" />
                </div>
                <h3 className="font-[family-name:var(--font-cormorant)] text-xl font-light text-[#111111] mb-2">
                  No Objects Selected
                </h3>
                <p className="text-xs text-[#595D5D] font-light max-w-xs mb-6">
                  Explore our restricted serial allocations to begin your domestic curation.
                </p>
                <button
                  onClick={closeCart}
                  className="px-6 py-3 bg-[#111111] text-[#f9f9f7] text-xs uppercase tracking-[0.2em] font-medium hover:bg-[#2b2b2b] transition-colors cursor-pointer"
                >
                  Return to Atelier
                </button>
              </div>
                  ...
                  <div className="flex-1 flex flex-col justify-between">
                    <div>
                      ...
                      {item.specifications?.gauge && (
                        <p className="text-xs uppercase tracking-[0.12em] text-[#595D5D] mt-1 font-light">
                          {item.specifications.gauge}
                        </p>
                      )}
                    </div>
                    ...
                    <div className="mt-3 pt-2 border-t border-[#f0f0ee]">
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center border border-[#d6d6d4] bg-[#ffffff]">
                          ...
                          <span className="px-3 text-xs font-medium text-[#111111]">
                            {item.quantity}
                          </span>
                          <button
                            onClick={() => updateQuantity(item.id, item.quantity + 1)}
                      ...
                      {item.quantity >= (maxPerProduct || 4) && (
                        <p className="text-xs text-[#595D5D] italic mt-1.5 font-light">
          ...
          {items.length > 0 && (
            <div className="border-t border-[#e5e5e3] bg-[#ffffff] p-6 space-y-4">
              <div className="space-y-2 text-xs uppercase tracking-[0.14em]">
              ...
              <div className="pt-2">
                <Link
                  href="/checkout"
                  onClick={closeCart}
                  className="w-full py-4 bg-[#111111] text-[#f9f9f7] hover:bg-[#2b2b2b] text-xs uppercase tracking-[0.2em] font-semibold flex items-center justify-center space-x-2 transition-all border border-[#111111]"
                >
