...
import type { Order, ShippingAddress } from '../../types/store';
...
export default function AccountPage() {
  ...
  useEffect(() => {
    async function loadUserOrders() {
      ...
      try {
        const list = await getOrdersByUser(user.uid, user.email || undefined);
        setOrders(list);
      } catch (err) {
    ...
    <div className="min-h-screen bg-[#f9f9f7] text-[#111111]">
      ...
      <main className="max-w-6xl mx-auto px-6 py-12">
        ...
        <div className="bg-[#ffffff] border border-[#e5e5e3] p-8 mb-8 flex flex-col md:flex-row md:items-center justify-between gap-6">
          ...
          <div className="flex items-center space-x-3 text-[11px] uppercase tracking-[0.14em]">
            <Link
              href="/admin"
              className="px-4 py-2.5 bg-[#ffffff] border border-[#d6d6d4] hover:border-[#111111] text-[#111111] font-medium transition-colors"
            >
